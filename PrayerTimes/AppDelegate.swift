...
class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        log("Application finished launching", level: .info, category: "Lifecycle")
        ...
        if isTestingMode {
            log("Running in testing mode", level: .info, category: "Startup")
        ...
        wakeObserver = NSWorkspace.shared.notificationCenter.addObserver(
            ...
            log("System woke from sleep", level: .info, category: "System")
            self?.systemDidWake()
        }
    ...
    private func systemDidWake() {
        log("System wake - updating prayer times", level: .info, category: "System")
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        log("Will present notification: \(notification.request.identifier)", level: .debug, category: "Notifications")
        completionHandler([.banner, .sound, .list, .badge])
    }

    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        log("Did receive notification response: \(response.actionIdentifier)", level: .debug, category: "Notifications")
    ...
    private func handleNotificationResponse(_ response: UNNotificationResponse, completionHandler: @escaping () -> Void) {
        ...
        guard let prayerName = userInfo["prayerName"] as? String else {
            log("Notification response missing prayerName", level: .warning, category: "Notifications")
            return
        }

        log("Handling notification action: \(actionIdentifier) for prayer: \(prayerName)", level: .info, category: "Notifications")
        ...
        case "DISMISS_ACTION":
            log("Notification dismissed by user", level: .debug, category: "Notifications")

        default:
            log("Unknown notification action: \(actionIdentifier)", level: .warning, category: "Notifications")
        }
    }

    private func snoozeNotification(prayerName: String, minutes: Int, userInfo: [AnyHashable: Any]) {
        log("Snoozing notification for \(prayerName) for \(minutes) minutes", level: .info, category: "Notifications")
    ...
    func applicationWillTerminate(_ notification: Notification) {
        log("Application will terminate", level: .info, category: "Lifecycle")
        NotificationManager.cancelNotifications()
        FullScreenNotificationManager.shared.dismissFullScreenNotification()
        AppLogger.shared.saveAppState([
            "terminatedGracefully": true,
            "lastVersion": Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "unknown"
        ])
    ...
    func applicationWillHide(_ notification: Notification) {
        log("Application will hide", level: .debug, category: "Lifecycle")
    }

    func applicationDidBecomeActive(_ notification: Notification) {
        log("Application became active", level: .debug, category: "Lifecycle")
    }
